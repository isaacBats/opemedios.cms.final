...
;(function($) {
	...
	var defaults = {
		...
		//Prototipo0 - RAT - 09/07/2014 - Se cambiÃ³ este parametro para que las flechas de navegaciÃ³n no descuadraran l lightbox
		fixedNavigation:		true,		// (boolean) Boolean that informs if the navigation (next and prev button) will be fixed or not in the interface.
		// Configuration related to images
		imageLoading:			'/imgs/lightbox-ico-loading.gif',		// (string) Path and the name of the loading icon
		imageBtnPrev:			'/imgs/Prev1.png',			// (string) Path and the name of the prev button image
		imageBtnNext:			'/imgs/Next1.png',			// (string) Path and the name of the next button image
		imageBtnClose:			'/imgs/btnCerrar2.png',		// (string) Path and the name of the close btn
		imageBlank:				'/imgs/blank.gif',			// (string) Path and the name of a blank image (one pixel)
	 ...
	 *
	 */
	//Prototipo0 - RAT - 09/07/2014 - Se modifica esta funciÃ³n para omitir la craciÃ³n de lightbox ya que ahora vive en la plantlla
	function _set_interface() {
		// Apply the HTML markup into body tag
		//$('body').append('<div id="jquery-overlay"></div><div id="jquery-lightbox"><div id="lightbox-container-image-box"><div id="lightbox-container-image"><div style="" id="lightbox-nav"><a href="#" id="lightbox-nav-btnPrev"></a><img id="lightbox-image" style="min-width:10%;"/><a href="#" id="lightbox-nav-btnNext"></a></div><div id="lightbox-loading"><a href="#" id="lightbox-loading-link"><img src="' + defaults.imageLoading + '"></a></div></div></div><div id="lightbox-container-image-data-box"><div id="lightbox-container-image-data"><div id="lightbox-image-details"><span id="lightbox-image-details-caption"></span><span id="lightbox-image-details-currentNumber"></span><span class="st_facebook_hcount" displayText="Facebook"></span><span class="st_twitter_hcount" displayText="Tweet"></span><span class="st_pinterest_hcount" displayText="Pinterest"></span> </div><div id="lightbox-secNav"><a href="#" id="lightbox-secNav-btnClose"><img src="' + defaults.imageBtnClose + '"></a></div></div></div></div>');	
		// Get page sizes
	...
	function _resize_container_image_box(intImageWidth,intImageHeight) {
		...
		//Prototipo0 - RAT - 09/07/2014 - ModificaciÃ³n para ajustar lightbox a tamaÃ±o de imagen
		var intWidth = (intImageWidth + 75 + (defaults.containerBorderSize * 2));//(intImageWidth + (defaults.containerBorderSize * 2)); // Plus the imageÂ´s width and the left and right padding value
		var intHeight = (intImageHeight + (defaults.containerBorderSize * 2)); // Plus the imageÂ´s height and the left and right padding value
		// Diferences
		...
		$('#lightbox-container-image-data-box').css({ width: intWidth - (defaults.containerBorderSize * 2) });
	...
	function changeThumbnail(activeImg) {
        ...
        //document.getElementById("idhash").value='#'+activeImg;
	...
	function _set_navigation() {
		$('#lightbox-nav').show();
		//Prototipo0 - RAT - 09/07/2014 - Se agrega esta lÃ­nea para asignar la imagen del boton de cerrado de lightbox
		$('#lightbox-close').attr('src',defaults.imageBtnClose);
		...
		if ( defaults.activeImage != 0 ) {
			if ( defaults.fixedNavigation ) {
				$('#lightbox-nav-btnPrev').css({ 'background' : 'url(' + defaults.imageBtnPrev + ') 10% 50% no-repeat' })
					.unbind()
					.bind('click',function() {
				...
				$('#lightbox-nav-btnPrev').unbind().hover(function() {
					$(this).css({ 'background' : 'url(' + defaults.imageBtnPrev + ') 10% 50% no-repeat' });
				},function() {
		...
		if ( defaults.activeImage != ( defaults.imageArray.length -1 ) ) {
			if ( defaults.fixedNavigation ) {
				$('#lightbox-nav-btnNext').css({ 'background' : 'url(' + defaults.imageBtnNext + ') 90% 50% no-repeat' })
					.unbind()
					.bind('click',function() {
						defaults.activeImage = defaults.activeImage + 1;
						changeThumbnail(defaults.activeImage);
				...
				$('#lightbox-nav-btnNext').unbind().hover(function() {
					$(this).css({ 'background' : 'url(' + defaults.imageBtnNext + ') 90% 50% no-repeat' });
				},function() {
					$(this).css({ 'background' : 'transparent url(' + defaults.imageBlank + ') no-repeat' });
				}).show().bind('click',function() {
					defaults.activeImage = defaults.activeImage + 1;
	...
	$.fn.galleriffic = function(thumbsContainerSel, settings) {
		...
		$.extend(this, {
			...
			ssAdvance: function() {
                ...
                //document.getElementById("idhash").value='#'+nextHash;
				// IE we need to explicity call goto
				if ($.browser.msie) {
					this.goto(nextIndex);
				}
			...
			refresh: function() {
				if (this.$imageContainer) {
					...
					var isFading = 1;
					var gallery = this;
                    //document.getElementById("idhash").value=this.currentIndex;
					// hide img
					this.$imageContainer.fadeOut('fast', function() {
                        ...
                        document.getElementById("navRight").href="#"+gallery.data[gallery.getNextIndex(gallery.currentIndex)].hash;
                        document.getElementById("navLeft").href="#"+gallery.data[gallery.getPrevIndex(gallery.currentIndex)].hash;
                        if(document.getElementById("navRightThumb")!=null)
                        {
                            document.getElementById("navRightThumb").href="#"+gallery.data[gallery.getNextIndex(gallery.currentIndex)].hash;
                        }
                        if(document.getElementById("navLeftThum")!=null)
                        {
			...
			buildImage: function(image) {
				if (this.$imageContainer) {
					...
					// Setup image
					//Prototipo0 - RAT - 09/07/2014 - Se modifica para agregar un span que ayude a centrar las imagenes
					//la lÃ­nea modificada es el append luego de ".find('a')" -> .append('<span style="display:inline-block; height:100%; vertical-align:middle"></span>',image)
					this.$imageContainer
						...
						.append('<span class="image-wrapper"><a class="advance-link fancybox" rel="history" title="'+image.alt+'" href="'+image.src+'"></a></span>')
						//.append('<span class="image-wrapper"><a class="" rel="history" title="'+image.alt+'"></a></span>')
						.find('a')
                        .append('<span style="display:inline-block; height:100%; vertical-align:middle"></span>',image)
						//.append(image).find('img').attr("id","imgOrig")
			...
			updateThumbs: function() {
				if (this.$thumbsContainer) {
					...
					for (i=startIndex; i<=stopIndex; i++) {
						...
						$ulThumbs.append('<li'+selected+'><a'+ sel +'  rel="history" href="#'+imageData.hash+'" title="'+imageData.title+'"><img src="'+imageData.thumb+'" alt="'+imageData.title+'" /></a></li>');
                        //$ulThumbs.append('<li'+selected+'><a rel="history" href="javascript:document.getElementById(\'imgOrig\').src=\''+this.data[i].original+'\';" title="'+imageData.title+'"><img src="'+imageData.thumb+'" alt="'+imageData.title+'" /></a></li>');
                        //$ulThumbs.append('<li'+selected+'><a rel="history" href="javascript:document.getElementById(\'idhash\').value=\'#'+imageData.hash+'\'" title="'+imageData.title+'"><img src="'+imageData.thumb+'" alt="'+imageData.title+'" /></a></li>');
		...
		if (this.settings.controlsContainerSel) {
            ...
            if (this.settings.renderNavControls) {					
				this.$controlsContainer
                    ...
                    .append('<div class="nav-controls"><a class="prev" rel="history" title="'+this.settings.prevLinkText+'"><img src="/imgs/prev2.png" /></a><a class="next" rel="history" title="Next"><img src="/imgs/next2.png" /></a></div>')
					.find('a[rel="history"]')
					.click(function() { clickHandler(gallery); });
			}*/
		}
...
/* GAM - 23/07/14 - BotÃ³n imprimir */

function printDiv(divID) {
            //Obtener el HTML del div
            var divElements = document.getElementById(divID).innerHTML;
            //Obtener el HTML de toda la pÃ¡gina
            var oldPage = document.body.innerHTML;

            //Establecer el HTML con el HTML del div solito
