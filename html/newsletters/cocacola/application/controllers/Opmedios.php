...
/*
     ...
     public function send()
	{
		...
		$bcc = explode(",", $data['newsletter']['emailBcc']);
		$config['protocol']  = 'sendmail';
		$config['smtp_user'] = 'cocacola.comunicacion@opemedios.mx';
		$config['smtp_pass'] = '1CL!UGq]A(w]';
		$config['smtp_host'] = 'mail.opemedios.mx';
		$config['smtp_port'] = '25';
		$config['mailtype']  = 'html';
		$config['charset']   = 'utf-8';
        ...
        $this->email->initialize($config);  
		$this->email->from('cocacola.comunicacion@opemedios.mx', ''.$data['newsletter']['nameCustomer'].'');
		$this->email->to(''.$data['newsletter']['emailTo'].'');
		$this->email->bcc($bcc);
